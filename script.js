...
function renderDashboard(){
  ...
  const totalDespesas = despesasP.reduce((s,d)=>s+Number(d.valor),0);
  ...
  const patrTotal = patrimonioTotal();
  ...
  const kpis = [
    ...
    {label:'Patrimônio', value: formatCurrency(patrTotal), icon:'gem', color:'blue'}
  ];
  document.getElementById('kpi-grid').innerHTML = kpis.map(kpiCardHtml).join('');

  renderDashboardCharts(despesasP, receitasP, start, end);
...
function renderDashboardCharts(despesasP, receitasP, start, end){
  const p = palette();
  ...
  ensureChart('chart-categorias', {
    ...
    options: baseChartOptions({legend:true, cutout:'65%'})
  });
  ...
  ensureChart('chart-metas', {
    ...
    options: baseChartOptions({legend:false, indexAxis:'y', max:100})
  });
}
...
function baseChartOptions(opts){
  ...
  return {
    ...
    plugins:{
      ...
      tooltip:{ backgroundColor:p.cardBg, titleColor:p.text, bodyColor:p.text, borderColor:p.border, borderWidth:1, padding:10, callbacks:{ label:(ctx)=> ' ' + formatCurrency(ctx.parsed.y!==undefined?ctx.parsed.y:ctx.parsed.x) } }
    },
...
function modalQuickAdd(){
  const opts = [
    ...
    {label:'Orçamento', action:'new-orcamento', icon:'pie-chart'},
    {label:'Compra parcelada', action:'new-parcelamento', icon:'layers'},
    {label:'Item de patrimônio', action:'new-patrimonio', icon:'gem'}
  ];
...
/* ============================== 21. EXPORTAR / LIMPAR DADOS ======================== */

function exportData(){
  const blob = new Blob([JSON.stringify(DB, null, 2)], {type:'application/json'});
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url; a.download = 'open-financing-backup.json';
  document.body.appendChild(a); a.click(); a.remove();
  URL.revokeObjectURL(url);
  toast('Backup exportado', 'Seus dados foram baixados em JSON.', 'success');
}
